...
public class GroupStrategyProvide extends ContentProvider {
	private static final String TAG = "GroupStrategyProvide";
	private final static String AUTHORITH = "com.vunke.chinaunicom.advertisement.groupstrategy";
...
//	private static final String _ID = "_id";
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		Cursor cursor = db.rawQuery("select "+ USERNAME +" from " + TABLE_NAME
				+ " where "+ USERNAME +" = '" + values.get(USERNAME) + "'", null);
		// db.query(true, TABLE_NAME, new String[] {
		// "body", "user_id", "create_time" }, null, null, "user_id", null,
		// null, null, null);
		if (cursor.moveToNext()) {// 有下一个 ，更新
			String userID = values.get(USERNAME).toString();
			db.update(TABLE_NAME, values, USERNAME +"=?", new String[] { userID });
		} else {// 否则 插入数据
			switch (mUriMatcher.match(uri)) {
			case 1:
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		case 2:
			...
			selection = (selection != null || "".equals(selection.trim()) ? USERNAME
					+ "=" + id
					: selection + "and" + USERNAME + "=" + id);
			number = db.delete(TABLE_NAME, selection, selectionArgs);
		}
		return number;
	}
